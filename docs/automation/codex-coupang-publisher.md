...
- 쿠팡 상품 후보를 조회한다.
- 가격대, 배송, 브랜드, 상품 차별점, 글 유형 적합도를 기준으로 상품을 선별한다.
...
HTML/SVG 자체 도식은 블로그 품질이 낮아 보이면 사용하지 않는다. 딥다이브 설명 이미지는 Pexels 사진 위에 HTML 텍스트 레이어를 합성한 JPEG로 만들고 WordPress 미디어에 업로드한다. 본문에는 Pexels 작가 출처 문구를 직접 노출하지 않고, 이미지 하단에는 해당 섹션의 구매 판단 의미만 짧게 붙인다. 일반 블로그, 뉴스, 리뷰 사이트, 쇼핑몰 상세페이지, 브랜드 공식몰 이미지는 명시적 사용 허가나 프레스킷 조건이 없으면 사용하지 않는다.
...
```text
1. 확인 팩트: 브랜드, 카테고리, 확인 가능한 스펙, 작성 시점 가격, 배송 조건
...
- 상품명, 가격, 이미지, 링크, 배송 조건, 카테고리, 상품 상세 페이지의 공개 메타 정보를 우선 사용한다.
...
- 저가형 생활상품은 로켓배송/무료배송을 강하게 우선한다.
- 중가형 상품은 로켓배송/무료배송을 중요 가점으로 본다.
- 프리미엄 상품은 배송보다 브랜드, 스펙, AS, 설치/보증 정보를 우선한다.
...
- 상품 정보 출처는 독자 관점에서 “작성 시점에 확인 가능한 상품명, 가격, 이미지, 링크, 배송 조건”처럼 표현한다.

TOP 3~5 비교 글의 상품별 섹션은 단순 설명문으로 끝내지 않는다. 각 상품마다 아래 요소를 포함한다.

```text
1. 핵심 판단: 가격대, 용량/스펙, 배송/관리 포인트
2. 장점: 구매를 당길 수 있는 강점 2~3개
3. 단점: 후회 방지를 위한 주의점 2~3개
...
900px 이하 화면에서 데스크톱 테이블을 그대로 축소하면 상품명이 눌리고 CTA가 화면 밖으로 밀리므로 사용하지 않는다. 모바일 카드에는 상품 이미지, 상품명, 가격, 바로가기 버튼만 남긴다. 가격과 버튼은 한 화면에서 바로 보여야 하며, 각 카드는 선과 여백으로 분리한다.
...
본문 상품 이미지는 각 상품 섹션의 제목 바로 아래에 배치한다. `alt`에는 상품명을 넣고, `loading="lazy"`를 사용한다.
...
이 명령은 `pending` 후보 1개만 처리하고 WordPress에는 `draft` 상태로 올리도록 Codex CLI에 지시한다. 목적은 공개 배치 전에 본문 디자인, 비교표, 모바일 카드, 이미지, CTA, 쿠팡 고지, 링크 구조를 사람이 확인하는 것이다.
