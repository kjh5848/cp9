...
function sanitizePublicWording(html) {
  ...
  next = next.replace(
    ...
    '작성 시점에 확인 가능한 상품명, 가격, 이미지, 링크, 로켓배송 표시를 기준으로 선별했고, 리뷰 수와 평점은 공개 본문에서 확인되지 않은 수치로 과장하지 않았습니다.',
  )
  next = next.replace(
    /이번 선별은 쿠팡 상품 검색 API에서 확인 가능한 상품명, 가격, 이미지, 파트너스 링크, 배송 신호를 기준으로 했고, 리뷰 수와 평점은 검색 응답에 없으므로 필수 게이트로 사용하지 않았습니다\./g,
    '이번 선별은 작성 시점에 확인 가능한 상품명, 가격, 이미지, 링크, 로켓배송 표시를 기준으로 했고, 리뷰 수와 평점은 공개 본문에서 확인되지 않은 수치로 단정하지 않았습니다.',
  )
  next = next.replace(
    ...
    '작성 시점에 확인 가능한 가격, 이미지, 링크, 카테고리, 로켓배송 표시를 기준으로 걸러낸 후보입니다. 리뷰와 평점은 공개 본문에서 확인되지 않은 수치로 단정하지 않고, 상세 페이지의 최신 후기와 스펙 확인으로 보완해야 합니다.',
  )
  next = next.replace(
    /아래 제품들은 쿠팡 상품 검색 API에서 확인 가능한 [^.。]+?후보입니다\.\s*쿠팡 검색 API 응답에는 [^.。]+?\./g,
    '아래 제품들은 작성 시점에 확인 가능한 가격, 이미지, 링크, 카테고리, 로켓배송 표시를 기준으로 걸러낸 후보입니다. 리뷰와 평점은 공개 본문에서 확인되지 않은 수치로 단정하지 않고, 상세 페이지의 최신 후기와 스펙 확인으로 보완해야 합니다.',
  )
  next = next.replace(
    ...
    '작성 시점에 확인 가능한 상품명, 가격, 상품 이미지, 링크, 로켓배송 표시를 기준으로 선별했습니다. 리뷰 수와 평점은 공개 본문에서 확인되지 않은 수치로 단정하지 않았고,',
  )
  next = next.replace(
    /쿠팡 파트너스 상품 검색 API에서 확인 가능한 상품명, 가격, 상품 이미지, 파트너스 링크, 로켓배송과 무료배송 신호를 기준으로 선별했습니다\./g,
    '작성 시점에 확인 가능한 상품명, 가격, 상품 이미지, 링크, 로켓배송 표시를 기준으로 선별했습니다.',
  )
  next = next.replace(
    /쿠팡 검색 API에서 확인 가능한 상품명, 가격, 이미지, 링크, 배송 신호를 바탕으로 20개를 선별했습니다\. 리뷰와 평점은 검색 응답에 없으므로 과장된 만족도 표현은 배제했습니다\./g,
    '작성 시점에 확인 가능한 상품명, 가격, 이미지, 링크, 로켓배송 표시를 바탕으로 20개를 선별했습니다. 리뷰와 평점은 공개 본문에서 확인되지 않은 수치로 단정하지 않고, 과장된 만족도 표현은 배제했습니다.',
  )
  next = next.replace(
