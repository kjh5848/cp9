...
\`docs/automation/codex-coupang-publisher.md\` 설계서를 기준으로 A 자동화만 수행한다.${batchSuffix} 이 프롬프트는 이미 \`tools/codex-publisher/autopublish-cli.mjs\`가 생성한 최종 실행 작업이다. 따라서 \`autopublish-cli.mjs\`, \`run-autopublish-launchd.sh\`, \`launchctl\`, \`codex exec\`를 다시 호출하지 말고, 현재 실행 안에서 후보 확인, 상품 조회, 본문 생성, WordPress 발행, 이력 기록, 원본 삭제를 직접 수행한다. CP9 내부 오토파일럿 파이프라인이나 프로젝트 LLM API 키를 사용하지 않는다. 비밀값은 \`frontend/.env.local\`에서만 읽고, 값을 출력하지 않는다. \`data/keyword-candidates.json\`에서 \`pending\` 후보 1개를 선택하고, 기념일/선물 후보는 독립 발행 카테고리로 유지하되 실제 상품 조회는 \`coupangSearchTerm\`의 상품군을 우선한다. 쿠팡 파트너스 상품 후보를 조회해 가격대, 브랜드, 상품 차별점, 글 유형 적합도, 판매처별 구성 차이로 선별한다. 리뷰 수와 평점은 필수 게이트로 사용하지 말고, 별도 검증 가능한 출처가 있을 때만 추가 가점으로 사용한다. 공개되는 WordPress 본문과 네이버 카페 요약에는 API, 검색 API, 응답, productImage, productUrl, productPrice 같은 데이터 수집 방식이나 내부 필드명을 절대 쓰지 않는다. 공개 본문에서는 “작성 시점에 확인 가능한 상품명, 가격, 이미지, 링크, 구성품”처럼 독자 관점 표현만 쓴다. 제목과 h1~h6에는 콜론(: 또는 ：)을 절대 쓰지 않고 발견되면 발행하지 않는다. GEO와 AI 스니펫 대응을 위해 모든 상품 설명에는 확인 팩트, 해석 문장, 구매 전 질문을 포함하고, 확인되지 않은 스펙·리뷰·평점·판매량은 만들지 않는다. 상품 리스트는 ItemList, Product, Offer 구조화 데이터와 본문 표시 정보가 서로 어긋나지 않게 작성한다. 생활가전과 청소 가전은 30대~60대 여성, 직장인 주부, 엄마, 친구에게 추천하는 사람의 관점으로 작성하고, 가격 확인 버튼 직전에는 실제 생활 장면의 고민과 후기에서 확인할 반복 불편을 먼저 설명한다. 상품 이미지는 본문 폭 안에서 중앙 정렬하고, 모바일에서 좌측으로 붙어 보이면 발행하지 않는다. 글 유형은 TOP 3~5 비교, 딥다이브, 20가지 큐레이션 비율을 설계서 기준으로 섞는다. Codex 자체 모델로 한국어 SEO 글을 작성한다. TOP 3~5 비교 글의 상품별 설명은 핵심 판단, 좋은 점, 조심할 점, 추천 대상, 구매 전 체크를 포함한다. 로봇청소기 글은 웹 리서치로 인기 브랜드와 반복 후기를 확인하고, 쿠팡뿐 아니라 공식몰, 대형가전몰, 오픈마켓의 구성 차이를 비교하도록 작성한다. 딥다이브는 공백 제외 8,000자 이상으로 작성하고 리포트/보고서형 서식으로 핵심 판단, 읽어야 할 사람, 가장 큰 리스크를 상단에 요약하되 글 제목을 반복하지 않는다. 딥다이브 본문은 문단만 나열하지 말고 목록, 인용, 잡지형 체크 카드, 짧은 위트 문장을 섞는다. 딥다이브 본문은 유명 브랜드, 대표 아이템, 브랜드 철학, 브랜드 이미지, 사용자가 얻는 의미를 중심으로 다루며 상품 링크는 구매 확인용 보조 영역으로만 둔다. 딥다이브 구매 확인 영역에는 쿠팡 파트너스 상품 이미지를 최소 3개 이상 넣고, 상품 이미지는 각 상품명 바로 아래에 배치한다. 딥다이브 설명 이미지는 Pexels 사진 위에 HTML 텍스트 레이어를 합성한 960x560 JPEG를 WordPress 미디어로 업로드해 본문 h2 아래에 최소 3개 이상 배치한다. 공개 본문 이미지는 Pexels 원본 URL로 링크하거나 Pexels 외부 이미지를 직접 로드하지 않는다. 본문에는 Photo by 작가명 on Pexels 문구를 노출하지 않고 sourceUrl, photographer, license는 history에만 남긴다. 일반 블로그, 뉴스, 리뷰 사이트, 쇼핑몰 상세페이지 이미지는 명시적 사용 허가가 없으면 사용하지 않는다. 20가지 큐레이션은 표를 쓰지 말고 모바일 우선 카드형 섹션으로 만들며 각 아이템마다 300자 내외 설명, 이미지, 상품명, 역할, 가격, 가격 확인 버튼을 포함한다. 대표 썸네일은 \`tools/codex-publisher/thumbnail-title.mjs\` 규칙을 사용하고, 원문 제목을 한 줄로 넣지 말며, 1200x630 overflow를 검증한 뒤 업로드한다. ${statusInstruction} \`COUPANG_ACCESS_KEY\`와 \`COUPANG_SECRET_KEY\`는 필수지만, \`COUPANG_PARTNER_ID\`는 선택값이므로 비어 있어도 발행을 막지 않는다. 딥다이브 발행에는 \`PEXELS_API_KEY\`가 필요하며 비어 있으면 딥다이브 회차는 발행하지 말고 누락을 보고한다. WordPress 카테고리는 후보 카테고리와 키워드 의도에 맞춰 CP 운영 카테고리(생활가전, 주방가전, 청소/생활, 기념일/선물, 가전·디지털, 식품·신선식품, 생활용품, 홈·인테리어, 뷰티, 주방용품, 패션, 반려동물, 출산·유아동, 스포츠·레저, 문구·오피스, 자동차용품, 완구·취미, 도서, 건강·의료) 중 하나의 하위 카테고리를 추가하고, 하위 카테고리 ID가 없으면 CP 루트 카테고리만 사용한다. 네이버 카페 환경변수가 모두 있으면 후보 카테고리에 맞는 메뉴 ID로 WordPress 원문 링크와 쿠팡 파트너스 고지 포함 요약 글을 카페에도 업로드한다. \`data/codex-publisher-history.json\`에는 키워드, 글 유형, 상품 id, WordPress post id, URL, 상태, 플랫폼별 결과, 딥다이브 Pexels 이미지 출처 메타데이터만 최소 기록한다. 필수 환경변수나 입력 파일이 없거나 품질 점수, 쿠팡 고지, 이미지, 링크, 표 구조가 미달이면 발행하지 말고 누락/오류를 보고한다. 성공 또는 실패 시 macOS 로컬 알림을 표시한다.`
}
