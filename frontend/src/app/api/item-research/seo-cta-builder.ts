...
`;
  ...
  const boxStyle = (() => {
    ...
    switch (layout) {
      case 'minimal':
        return `padding:16px 0;margin:24px 0;${flexStyles}`;
      case 'banner':
        return `padding:24px 20px;background:${boxBg};border:none;border-left:6px solid ${btnColor};border-radius:8px;${shadow}margin:24px 0;${flexStyles}`;
      case 'gradient':
        return `padding:24px 20px;background:linear-gradient(135deg, ${boxBg}, ${btnColor});border-radius:16px;${shadow}margin:24px 0;${flexStyles}`;
      case 'outline':
        return `padding:20px;background:transparent;border:2px solid ${boxBorder};border-radius:12px;${shadow}margin:24px 0;${flexStyles}`;
      case 'shadow':
        return `padding:24px;background:${boxBg};border:none;border-radius:20px;${strongShadow}margin:24px 0;${flexStyles}`;
      case 'neon':
        return `padding:20px;background:#0f172a;border:1px solid ${btnColor};border-radius:12px;box-shadow:0 0 15px ${btnColor}40, inset 0 0 20px ${btnColor}20;margin:24px 0;color:#fff;${flexStyles}`;
      case 'coupon':
        return `padding:24px 20px;background:${boxBg};border:2px dashed ${boxBorder};border-radius:8px;margin:24px 0;position:relative;${flexStyles}`;
      case 'modern':
        return `padding:24px 0;background:transparent;border-top:1px solid ${boxBorder};border-bottom:1px solid ${boxBorder};margin:24px 0;${flexStyles}`;
      case 'luxury':
        return `padding:24px 20px;background:#171717;border:1px solid #333;border-radius:16px;${shadow}margin:24px 0;color:#f3f4f6;${flexStyles}`;
      case 'card':
      default:
        return `padding:20px;background:${boxBg};border:1px solid ${boxBorder};border-radius:16px;${shadow}margin:24px 0;${flexStyles}`;
    }
  })();
  ...
  const btnStyleObj = (() => {
    switch (layout) {
      case 'gradient':
        return `${baseBtnStyle}background:rgba(255,255,255,0.95);color:${btnColor};`;
      case 'outline':
        return `${baseBtnStyle}background:transparent;color:${btnColor};border:2px solid ${btnColor};`;
      case 'neon':
        return `${baseBtnStyle}background:transparent;color:${btnColor};border:1px solid ${btnColor};box-shadow:0 0 10px ${btnColor}80;`;
      case 'luxury':
        return `${baseBtnStyle}background:${btnColor};color:${btnText};box-shadow:0 4px 6px -1px rgba(0,0,0,0.5);`;
      default:
        return `${baseBtnStyle}background:${btnColor};color:${btnText};`;
    }
  })();
