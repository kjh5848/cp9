...
[목표 글자수 및 분량 제한 - 필수]: 
- 이 글은 공백을 포함하여 **반드시 전체 ${charLimit}자 내외**로 작성되어야 합니다.
- 한국어 1글자 = 1자로 계산합니다. 목표 분량인 ${charLimit}자를 절대 무리하게 초과하지 마세요. (예: 3000자가 제시되었는데 7000자로 쓰는 행위 등은 엄격히 금지됨)
- 지정된 분량 내에서 핵심만 간결하게 전달하고, 글이 지루하게 길어지는 것을 방지하십시오. 목표 글자수에 도달하면 즉시 글을 마무리하세요.
...
- **[금지어 및 기호 규칙 - 필수 준수]**:
  ...
  - 제목, 소제목, 본문 등 글의 모든 부분에서 콜론(:) 기호 사용 절대 금지. (단, 매크로 '[[[CTA_BUTTON:...]]]' 및 '[이미지 제안: ...]' 에는 예외적으로 허용)
...
- **[중요 필수]** 주제가 전환되는 주요 섹션(H2, H3)마다 본문 사이에 반드시 \`[이미지 제안: 검색어/상황 묘사]\` 형태의 텍스트를 삽입하라.
...
- 마크다운 취소선(~~텍스트~~)은 절대 사용하지 마라.
- **[중요 필수]** 주제가 전환되는 주요 섹션(H2, H3)마다 본문 사이에 반드시 \`[이미지 제안: 상품명/형태 묘사]\` 형태의 텍스트를 삽입하라.
...
[목표 글자수 및 분량 제한 - 필수]: 
- 이 글은 공백을 포함하여 **총 ${totalCharLimit}자 내외**로 작성되어야 하며, 아이템당 권장 글자수는 약 ${perItemChars}자 입니다.
- 한국어 1글자 = 1자로 계산합니다. 전체 분량인 ${totalCharLimit}자를 절대 초과하지 않도록 각 상품 소개를 핵심 위주로 짧게 요약하세요. 
- 글이 지루하게 길어지는 것을 방지하십시오. 목표 분량에 도달하면 즉시 글을 조기 마무리하세요.
...
- 마크다운 취소선(~~텍스트~~)은 절대 사용하지 마라.
- **[중요 필수]** 각 상품 소개 섹션마다 본문 사이에 반드시 \`[이미지 제안: 상품명/특징 묘사]\` 형태의 텍스트를 삽입하라.
...
- 이 글은 공백을 포함하여 **반드시 전체 ${charLimit}자 내외**로 작성되어야 합니다.
- 한국어 1글자 = 1자로 계산합니다. 목표 분량인 ${charLimit}자를 절대 무리하게 초과하지 마세요. (예: 3000자가 제시되었는데 7000자로 쓰는 행위 등은 엄격히 금지됨)
- 지정된 분량 내에서 핵심만 간결하게 전달하고, 단어와 문맥을 장황하게 늘려 쓰지 마십시오. 목표 분량에 도달하면 즉시 글을 조기 마무리하세요.
