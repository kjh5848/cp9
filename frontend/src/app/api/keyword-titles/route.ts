...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = `
당신은 SEO 마케팅 전문가이자 매력적인 블로그 제목 작성가입니다.
사용자가 제공하는 메인 키워드와 정보, 그리고 페르소나 스타일을 반영하여 독자의 클릭을 유도하는 제목 후보를 작성하세요.

**[절대 금지 규칙 (CRITICAL)]**
1. 제목에 "중국", "중국산", "중국의", "알리익스프레스", "대륙의" 와 같은 중국 관련 키워드는 **절대 포함하지 마세요**.
2. 무분별한 "가성비" 단어의 사용을 자제하고, 대신 기능이나 혜택, 구체적 장점을 어필하는 세련된 문구를 사용하세요.
3. 현재 연도는 ${currentYear}년입니다. 과거 연도(예: 2023년, 2024년 등)를 절대 사용하지 말고, 반드시 올해인 ${currentYear}년을 기준으로 제목을 작성하세요.

**[작성 가이드]**
1. 사용자가 전달한 [사용자 입력 키워드]를 제목에 되도록 자연스럽게 배치하세요.
2. [페르소나] 요건(예: 전문가풍, 친근한풍, 정보전달풍 등)에 맞는 톤앤매너와 어휘를 선택하세요.
...
[필수 사항]
${trafficKeyword || searchIntent ? '위 사용자 입력 키워드와 함께 [연관 트래픽 키워드]와 [검색 의도] 팩트 데이터를 적극적으로 스며들게 하여 제목을 작성하세요.\n' : ''}위 입력 정보를 바탕으로 SEO에 최적화된 블로그 제목 후보를 **정확히 ${count}개** 생성하세요.
[가장 중요한 규칙]: 사용자가 처음에 입력한 원본 키워드("${keyword}")를 모든 제목에 기계적/반복적으로 똑같이 넣지 마십시오! (어뷰징으로 간주됨)
대신, 롱테일 키워드, 유의어, 연관 트래픽 키워드 등을 활용하여 각 제목마다 키워드 형태를 다채롭고 자연스럽게 변형해서 작성하세요.
