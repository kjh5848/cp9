    ...
    <div className={cn("p-6 bg-card/50 backdrop-blur-md rounded-2xl border border-border/50 shadow-xl", className)}>
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/10">
        {products.map((p) => (
          <div
            key={p.productId}
            className="group relative flex-none w-28 rounded-lg border border-slate-700/50 overflow-hidden bg-slate-800/20 transition-all hover:border-slate-500/50"
          >
            {onRemove && (
              <button
                onClick={(e) => {
                ...
                className="absolute -top-1 -right-1 z-20 w-5 h-5 bg-red-500 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
              >
            ...
            <div className="relative w-full aspect-square bg-slate-900/50">
              {p.productImage ? (
                ...
                <Package className="w-8 h-8 text-slate-700 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
              )}
            </div>
            
            <div className="p-2.5">
              <p
                className="text-[10px] text-slate-300 line-clamp-2 leading-tight mb-1"
                title={p.productName}
              >
                {p.productName}
              </p>
