            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <GlassCard className="p-6">
                ...
                <div className="relative">
                  <input id="keyword-input" type="text" placeholder="예: 로봇청소기 추천 2026, 신혼가전 패키지..." className="w-full bg-background/50 border border-border rounded-xl px-4 py-3.5 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-blue-500/40 text-[15px]" value={s.keyword} onChange={(e) => a.setKeyword(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && s.keyword.trim()) { a.setStepA(1); if (s.titles.length === 0) a.handleGenerateTitles(); } }} />
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              <GlassCard className="p-6">
                <div className="flex items-center gap-3 mb-4"><div className="p-2 bg-orange-500/20 rounded-lg"><ShoppingCart className="w-5 h-5 text-orange-400" /></div>
                  ...
                  {s.selectedProductIds.size > 0 && <div className="bg-blue-500/20 text-blue-400 text-xs font-bold px-3 py-1 rounded-full">{s.selectedProductIds.size}개 선택</div>}
                </div>
              ...
              {s.isSearchingCoupang && <div className="flex flex-col items-center py-12 gap-3"><Loader2 className="w-8 h-8 animate-spin text-orange-500" /><p className="text-sm text-muted-foreground">쿠팡 검색 중...</p></div>}
              {!s.isSearchingCoupang && s.coupangResults.length > 0 && <ProductGrid products={s.coupangResults} selectedIds={s.selectedProductIds} onToggle={a.toggleProduct} />}
              {!s.isSearchingCoupang && s.coupangResults.length === 0 && !s.deepLinkResult && <div className="text-center py-10"><Package className="w-10 h-10 text-muted-foreground/30 mx-auto mb-2" /><p className="text-sm text-muted-foreground">검색 결과 없음. 검색 조건을 수정해보세요.</p></div>}
              <div className="flex justify-between items-center">
                <Button variant="ghost" onClick={() => a.setStepA(1)}><ChevronLeft className="w-4 h-4 mr-1" />이전</Button>
                <div className="flex gap-2">
                  {s.selectedProductIds.size === 0 && <Button onClick={() => a.setStepA(3)} variant="ghost" className="text-muted-foreground text-sm"><SkipForward className="w-4 h-4 mr-1" />건너뛰기</Button>}
          ...
          {s.stepB === 0 && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              {!s.isSearchingCoupang && s.coupangResults.length > 0 && (
                <>
                  <div className="flex items-center justify-between px-1"><span className="text-xs text-muted-foreground">{s.coupangResults.length}개 결과</span>{s.selectedProductIds.size > 0 && <span className="text-xs text-blue-400 font-bold">{s.selectedProductIds.size}개 선택됨</span>}</div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {s.selectedProducts.map((p, idx) => (
                    <div key={idx} className="flex-none w-28 rounded-lg border border-border/30 overflow-hidden">
                      <div className="relative w-full aspect-square bg-white/5">{p.productImage && <Image src={p.productImage} alt={p.productName} fill sizes="112px" className="object-cover" unoptimized />}</div>
