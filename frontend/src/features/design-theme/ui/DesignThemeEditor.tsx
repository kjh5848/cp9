                ...
                <div className="space-y-4">
                  ...
                  <Accordion type="multiple" className="space-y-2">
                    {(config.ctaBlocks || []).map((block) => {
                      ...
                      return (
                      <AccordionItem key={block.id} value={block.id} className="border border-slate-700 rounded-lg bg-slate-800/30 overflow-hidden px-3">
                        ...
                        <AccordionContent className="pt-2 pb-4 space-y-4 px-1 border-t border-slate-700/50">
                          ...
                          <div className="flex gap-2">
                            <div className="flex-1">
                              ...
                              <Input value={block.name} onChange={e => updateBlock({ name: e.target.value })} className="h-7 text-xs bg-slate-900 border-slate-700 text-slate-300" />
                            </div>
                            <Button size="icon" variant="outline" onClick={() => actions.updateConfig('ctaBlocks', (config.ctaBlocks || []).filter(b => b.id !== block.id))} className="h-7 w-7 mt-auto border-red-500/30 text-red-400 hover:bg-red-500/10">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <select value={block.placement.position} onChange={e => updatePlacement({ position: e.target.value })} className="h-7 w-full px-2 text-xs bg-slate-900 border border-slate-700 text-slate-300 rounded focus:outline-none focus:border-blue-500">
                                <option value="before-h2">첫 번째 H2 이전 (최상단)</option>
                                <option value="after-h2">모든 H2 로드 직후</option>
                                <option value="before-h3">모든 H3 로드 직전</option>
                                <option value="after-h3">모든 H3 로드 직후</option>
                                <option value="random-p">문단(p) 무작위 삽입</option>
                                <option value="article-end">글 최하단</option>
                              </select>
                            </div>
                            <div>
