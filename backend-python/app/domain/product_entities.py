...
class ProductReviews:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.rating_avg <= 5:
            raise ValueError(f"Rating average must be between 0 and 5, got {self.rating_avg}")
...
class ProductResearchResult:
    ...
    captured_at: str = field(default_factory=lambda: datetime.utcnow().strftime("%Y-%m-%d"))
    ...
    def is_valid(self) -> bool:
        ...
        return (
            self.status == ResearchStatus.SUCCESS and
            self.reviews.rating_avg > 0 and
            self.reviews.review_count > 0 and
            len(self.sources) >= 3
        )
...
class ProductResearchJob:
    ...
    def add_result(self, result: ProductResearchResult) -> None:
        ...
        elif result.status in [ResearchStatus.ERROR, ResearchStatus.INSUFFICIENT_SOURCES]:
            self.failed_items += 1
        self.updated_at = datetime.utcnow()
