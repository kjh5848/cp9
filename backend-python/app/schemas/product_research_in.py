...
class ProductItemRequest(BaseModel):
    ...
    price_exact: float = Field(..., gt=0, description="정확한 제품 가격", examples=[388000])
    currency: str = Field(
        default=DEFAULT_CURRENCY, description="통화 단위", examples=["KRW", "USD"]
    )
    seller_or_store: Optional[str] = Field(
        None, max_length=200, description="판매자 또는 스토어명", examples=["쿠팡", "11번가"]
    )

    # Coupang API 실제 구조 기반 필드들
    product_id: Optional[int] = Field(
        None, description="쿠팡 제품 ID (productId)", examples=[7582946]
    )
    product_image: Optional[str] = Field(
        None,
        description="쿠팡 제품 이미지 URL (productImage)",
        examples=[
            "https://thumbnail10.coupangcdn.com/thumbnails/remote/492x492ex/image/retail/images/..."
        ],
    )
    ...
    is_rocket: Optional[bool] = Field(
        None, description="로켓배송 여부 (isRocket)", examples=[True]
    )
    is_free_shipping: Optional[bool] = Field(
        None, description="무료배송 여부 (isFreeShipping)", examples=[True]
    )
    category_name: Optional[str] = Field(
        None, description="쿠팡 카테고리명 (categoryName)", examples=["이어폰/헤드폰"]
    )
...
class ResearchOptions(BaseModel):
    ...
    include_coupang_price: bool = Field(default=True, description="쿠팡 가격 비교 포함 여부")
    include_reviews: bool = Field(default=True, description="리뷰 정보 포함 여부")
    include_specs: bool = Field(default=True, description="상세 스펙 포함 여부")
    min_sources: int = Field(default=3, ge=1, le=10, description="최소 출처 개수")
    max_concurrent: int = Field(default=5, ge=1, le=10, description="최대 동시 요청 수")
...
class ProductResearchRequest(BaseModel):
    ...
    priority: int = Field(default=5, ge=1, le=10, description="작업 우선순위 (1=낮음, 10=높음)")

    @field_validator("items")
    @classmethod
    def validate_items_count(
        cls, v: List[ProductItemRequest]
    ) -> List[ProductItemRequest]:
...
class JobStatusRequest(BaseModel):
    ...
    include_details: bool = Field(default=True, description="상세 결과 포함 여부")
    include_failed: bool = Field(default=True, description="실패한 아이템 포함 여부")
