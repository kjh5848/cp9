...
class CoupangInfoResponse(BaseModel):
    ...
    # 가격 비교 정보
    price_comparison: Optional[Dict[str, float]] = Field(None, description="가격 비교 정보")
...
class ProductAttributeResponse(BaseModel):
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {"name": "포트", "value": "USB 3.0 x2, Mini HDMI, Type-C"}
        }
...
class ProductSpecsResponse(BaseModel):
    ...
    main: List[str] = Field(default_factory=list, description="주요 스펙 목록")
    attributes: List[ProductAttributeResponse] = Field(
        default_factory=list, description="상세 속성 목록"
    )
    size_or_weight: Optional[str] = Field(None, description="크기 또는 무게")
    options: List[str] = Field(default_factory=list, description="옵션 목록")
    included_items: List[str] = Field(default_factory=list, description="포함 구성품")
...
class ProductReviewsResponse(BaseModel):
    ...
    rating_avg: float = Field(0.0, ge=0, le=5, description="평균 평점 (5점 만점)")
    review_count: int = Field(0, ge=0, description="리뷰 개수")
    summary_positive: List[str] = Field(default_factory=list, description="긍정적 리뷰 요약")
    summary_negative: List[str] = Field(default_factory=list, description="부정적 리뷰 요약")
    notable_reviews: List[NotableReviewResponse] = Field(
        default_factory=list, description="주목할 만한 리뷰"
    )
...
class ProductResultResponse(BaseModel):
    ...
    # 쿠팡 정보 (실제 API 구조 기준)
    coupang_info: Optional[CoupangInfoResponse] = Field(None, description="쿠팡 제품 정보")

    specs: ProductSpecsResponse = Field(
        default_factory=ProductSpecsResponse, description="제품 스펙"
    )
    reviews: ProductReviewsResponse = Field(
        default_factory=ProductReviewsResponse, description="리뷰 정보"
    )
    sources: List[str] = Field(default_factory=list, description="정보 출처")
    captured_at: str = Field(..., description="정보 수집 날짜")
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "reviews": {"rating_avg": 4.3, "review_count": 41},
                "sources": [
                    "https://basic-s.com",
                    "https://www.enuri.com",
...
class ProductResearchResponse(BaseModel):
    ...
    results: List[ProductResultResponse] = Field(
        default_factory=list, description="리서치 결과 목록"
    )
    metadata: ResearchMetadataResponse = Field(..., description="작업 메타데이터")
...
class JobStatusResponse(BaseModel):
    ...
    job_id: UUID = Field(..., description="작업 ID")
    status: str = Field(..., description="작업 상태")
    progress: float = Field(0.0, ge=0, le=1, description="진행률 (0-1)")
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "details": {"max_allowed": 10, "received": 15},
            }
        }
