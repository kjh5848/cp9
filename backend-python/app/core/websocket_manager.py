...
class ConnectionManager:
    ...
    async def send_message_to_job(
        ...
        connections = self.job_connections[job_id].copy()  # Avoid modification during iteration
        disconnected_connections = []
        
        for websocket in connections:
            try:
                await websocket.send_text(message.json())
                logger.debug(f"Sent message to WebSocket for job {job_id}: {message.type}")
                
            except WebSocketDisconnect:
                logger.info(f"WebSocket disconnected during message send for job {job_id}")
                disconnected_connections.append(websocket)
                
            except Exception as e:
                logger.error(f"Failed to send message to WebSocket for job {job_id}: {e}")
                disconnected_connections.append(websocket)
    ...
    async def send_job_progress_update(
        ...
        progress_percentage = (current_item / total_items * 100) if total_items > 0 else 0
