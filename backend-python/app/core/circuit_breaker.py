...
class CircuitBreaker:
        ...
        with self._lock:
            ...
            if self._state == CircuitState.HALF_OPEN:
                ...
                if self._success_count >= self.config.success_threshold:
                    logger.info(
                        f"Circuit breaker for {self.service_name} moving to CLOSED"
                    )
                    self._state = CircuitState.CLOSED
                    self._failure_count = 0
        ...
        with self._lock:
            ...
            if self._state in [CircuitState.CLOSED, CircuitState.HALF_OPEN]:
                if self._failure_count >= self.config.failure_threshold:
                    logger.error(
                        f"Circuit breaker for {self.service_name} moving to OPEN"
                    )
    ...
    def _check_error_rate(self) -> None:
        ...
        if error_rate >= self.config.error_rate_threshold:
            ...
            if failures >= self.config.max_failures_per_window:
                logger.error(
                    f"Circuit breaker for {self.service_name} opening due to high error rate"
                )
                self._state = CircuitState.OPEN
    ...
    def _cleanup_old_calls(self) -> None:
        ...
        self._call_history = [
            call for call in self._call_history if call[0] > cutoff_time
        ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_response_time = 0
            if recent_calls:
                avg_response_time = sum(call[2] for call in recent_calls) / len(
                    recent_calls
                )
            ...
            return {
                ...
                "error_rate": recent_failures / len(recent_calls)
                if recent_calls
                else 0,
...
class CircuitBreakerRegistry:
    ...
    @classmethod
    def get_breaker(
        cls, service_name: str, config: Optional[CircuitBreakerConfig] = None
    ) -> CircuitBreaker:
