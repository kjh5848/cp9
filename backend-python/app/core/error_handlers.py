...
class GlobalExceptionHandler:
    ...
    async def base_api_exception_handler(
        ...
        response_data[
            "timestamp"
        ] = "2024-01-01T00:00:00Z"  # Would use actual timestamp
...
def setup_error_handlers(app) -> None:
    ...
    app.add_exception_handler(
        BaseAPIException, GlobalExceptionHandler.base_api_exception_handler
    )

    # FastAPI HTTP exceptions
    app.add_exception_handler(
        HTTPException, GlobalExceptionHandler.http_exception_handler
    )

    app.add_exception_handler(
        StarletteHTTPException, GlobalExceptionHandler.http_exception_handler
    )

    # Validation exceptions
    app.add_exception_handler(
        RequestValidationError, GlobalExceptionHandler.validation_exception_handler
    )

    # Generic exceptions (catch-all)
    app.add_exception_handler(
        Exception, GlobalExceptionHandler.generic_exception_handler
    )
    ...
    def create_frontend_friendly_error(
        ...
        response = {
            "error": {
                ...
                "recommended_action": ErrorConfig.ACTION_MAP.get(
