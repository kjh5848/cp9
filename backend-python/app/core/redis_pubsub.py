...
class RedisPubSubManager:
        ...
        try:
            async for message in self.pubsub.listen():
                ...
                try:
                    ...
                    logger.debug(
                        f"Received job update via PubSub: {job_update.job_id} - {job_update.message_type}"
                    )
    ...
    async def _forward_to_websocket(
        self, connection_manager, job_update: JobUpdateMessage
    ):
        ...
        try:
            if job_update.message_type == "status":
                ...
                await connection_manager.send_job_completion(
                    ...
                    total_processing_time_ms=job_update.data.get(
                ...
                await connection_manager.send_job_error(
                    ...
                    error_message=job_update.data.get(
                ...
                logger.warning(
                    f"Unknown job update message type: {job_update.message_type}"
                )
    ...
    async def publish_job_update(
        self, job_id: str, message_type: str, data: Dict[str, Any]
    ):
        ...
        try:
            message = JobUpdateMessage(
                job_id=job_id, message_type=message_type, data=data
            )

            await self.redis_client.publish("job_updates", message.json())
...
async def publish_job_completion(
    job_id: str, status: str, results_count: int, total_processing_time_ms: int
):
